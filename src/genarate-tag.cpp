    ...
    for (size_t i = 0; i < cur_page.size(); i++)
    {
        ...
        if (is60page(getFirstTow(tow)))
        {
            if (pageNum==4)
            {
                ...
                navBin.replace(170,8,hexToBinary(sss));
                cur_page[i].navMsg=binaryToHexBitset(navBin);    

                i++;
                navBin=hexToBinary(cur_page[i].navMsg);
                navBin.replace(146,4,"0100");
                navBin.replace(150,4,"0001");
            }
            if (pageNum==8)
            {
                string sss;
                ...
                navBin.replace(154,8,hexToBinary(sss));
                navBin.replace(162,4,"1100");
                navBin.replace(166,4,"0001");
                cur_page[i].navMsg=binaryToHexBitset(navBin);  
            }  
        }
        else{
