...
// Normalize the seconds, milliseconds, and fractional-millisecond fields in gal_sow_t.
static void normalizeGalSow(gal_sow_t *sow)
{
    // The fractional part should stay within one millisecond.
    if (sow->sub_msec >= 1.0 || sow->sub_msec <= -1.0)
    {
        double carry = floor(sow->sub_msec);
        sow->msec += (int)carry;
        sow->sub_msec -= carry;
    }

    // Keep the fractional part non-negative.
    while (sow->sub_msec < 0.0)
    {
        sow->sub_msec += 1.0;
        sow->msec--;
    }

    // Keep msec as a millisecond-within-second field.
    if (sow->msec >= 1000 || sow->msec <= -1000)
    {
        int carry = sow->msec / 1000;
        sow->sec += carry;
        sow->msec -= carry * 1000;
    }

    // Final borrow correction after millisecond normalization.
    while (sow->msec < 0)
    {
        sow->msec += 1000;
        sow->sec--;
...
// Convert galileo_time to the intermediate gal_sow_t representation.
gal_sow_t galSowFromGalTime(galtime_t g)
{
    gal_sow_t sow;

    sow.sec = (int)floor(g.sec);
    double msec = (g.sec - (double)sow.sec) * 1000.0;
    sow.msec = (int)floor(msec);
    sow.sub_msec = msec - (double)sow.msec;
...
// Convert the intermediate representation back to galileo_time.
galtime_t galSowToGalTime(gal_sow_t *sow, int *week)
{
    galtime_t g;
    const int seconds_in_week = (int)SECONDS_IN_WEEK;

    normalizeGalSow(sow);

    while (sow->sec >= seconds_in_week)
    {
        sow->sec -= seconds_in_week;
        (*week)++;
    }
    ...
    g.week = *week;
    g.sec = (double)sow->sec + (double)sow->msec / 1000.0 + sow->sub_msec / 1000.0;

    return g;
}

// Increment by milliseconds.
void incGalSowMillis(gal_sow_t *sow, int msec)
{
    sow->msec += msec;
    normalizeGalSow(sow);
}
...
void incGalSowSeconds(gal_sow_t *sow, double sec)
{
    double int_sec;
    double frac_sec = modf(sec, &int_sec);

    sow->sec += (int)int_sec;
    sow->msec += (int)floor(frac_sec * 1000.0);
    sow->sub_msec += frac_sec * 1000.0 - floor(frac_sec * 1000.0);
