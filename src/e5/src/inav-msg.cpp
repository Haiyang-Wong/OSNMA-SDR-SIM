...
static e5b_time_index_t e5bTimeIndexFromTransmitTime(double transmit_time)
{
    e5b_time_index_t idx = {};
    double tow = transmit_time;
    ...
    long page_idx = (long)floor(tow / (double)PAGE_TRANS_TIME);
    double page_start_sec = (double)page_idx * (double)PAGE_TRANS_TIME;
    idx.tow = (int)page_start_sec;
    return idx;
}

static uint64_t scaleE5bSqrtA(ephem_t *eph)
{
    return (uint64_t)UnscaleULong(eph->sqrta, -19);
}

static void writeE5bNavPage(galtime_t g, ephem_t *eph, int *page)
{
    static bool initialized = false;
    const char *nav_page_file = "/home/daohangzu/workplace/galileo-sdr-sim/nav_page_e5b.txt";
    FILE *fp = fopen(nav_page_file, initialized ? "a" : "w");
    ...
    initialized = true;
    fprintf(fp, "%d,%d,%d,", (int)g.sec, g.week, eph->svid);

    int txt_page[240] = {};
    memcpy(txt_page, page, 114 * sizeof(int));
    memcpy(txt_page + 120, page + 114, 114 * sizeof(int));

    for (int i = 0; i < 240; i += 4)
    {
        int value = ((txt_page[i] & 1) << 3) |
                    ((txt_page[i + 1] & 1) << 2) |
                    ((txt_page[i + 2] & 1) << 1) |
                    (txt_page[i + 3] & 1);
        fprintf(fp, "%X", value);
...
int generateINavMsg_E5b(galtime_t g, channel_t *chan, ephem_t *eph, ionoutc_t *ionoutc)
{
    int *page = (int *)malloc(PAGE_SIZE * sizeof(int));
    int *odd_page = (int *)malloc(120 * sizeof(int));
    int *even_page = (int *)malloc(120 * sizeof(int));
    ...
    int word_type_index = ((int)g.sec % 60) / 2;
    int word_type = WordAllocationE5b[word_type_index];

    generate_page_E5b(g, eph, ionoutc, even_page, odd_page, word_type);
    generateFrame(even_page, &page[0]);
    generateFrame(odd_page, &page[250]);
...
void generate_page_E5b(galtime_t g, ephem_t *eph, ionoutc_t *ionoutc, int *even_page, int *odd_page, int word_type)
{
    int offset = 0;
    int page[240] = {};
    ...
    e5b_time_index_t idx = e5bTimeIndexFromTransmitTime(g.sec);
    int TOW = idx.tow;
    ...
    {
    case 0:
        encode_int_to_bits(page, &offset, 0, 8);
        encode_int_to_bits(page, &offset, 2, 2);
        encode_int_to_bits(page, &offset, 0, 88);
    ...
    case 1:
        encode_int_to_bits(page, &offset, 1, 8);
        encode_int_to_bits(page, &offset, eph->iode, 10);
        IntValue = (int)eph->toe.sec / 60;
        encode_int_to_bits(page, &offset, IntValue, 14);
        IntValue = UnscaleInt(eph->m0 / PI, -31);
        encode_double_to_bits(page, &offset, IntValue, 32);
        UintValue = UnscaleUint(eph->ecc, -33);
        encode_uint64_to_bits(page, &offset, UintValue, 32);
        encode_uint64_to_bits(page, &offset, scaleE5bSqrtA(eph) & 0xffffffffULL, 32);
        encode_int_to_bits(page, &offset, 0, 2);
        break;

    case 2:
        encode_int_to_bits(page, &offset, 2, 8);
        encode_int_to_bits(page, &offset, eph->iode, 10);
        IntValue = UnscaleInt(eph->omg0 / PI, -31);
        encode_double_to_bits(page, &offset, IntValue, 32);
        IntValue = UnscaleInt(eph->inc0 / PI, -31);
    ...
    case 3:
        encode_int_to_bits(page, &offset, 3, 8);
        encode_int_to_bits(page, &offset, eph->iode, 10);
        IntValue = UnscaleInt(eph->omgdot / PI, -43);
        encode_int_to_bits(page, &offset, IntValue, 24);
        IntValue = UnscaleInt(eph->deltan / PI, -43);
    ...
    case 4:
        encode_int_to_bits(page, &offset, 4, 8);
        encode_int_to_bits(page, &offset, eph->iode, 10);
        encode_int_to_bits(page, &offset, eph->svid, 6);
        IntValue = UnscaleInt(eph->cic, -29);
        encode_int_to_bits(page, &offset, IntValue, 16);
    ...
    case 5:
        encode_int_to_bits(page, &offset, 5, 8);
        UintValue = UnscaleUint(ionoutc->ai0, -2);
        encode_double_to_bits(page, &offset, UintValue, 11);
        IntValue = UnscaleInt(ionoutc->ai1, -8);
        encode_double_to_bits(page, &offset, IntValue, 11);
        ...
        IntValue = UnscaleInt(eph->bgde5b, -32);
        encode_int_to_bits(page, &offset, IntValue, 10);
        encode_int_to_bits(page, &offset, eph->svhlth >> 7, 2);
        encode_int_to_bits(page, &offset, eph->svhlth >> 1, 2);
        encode_int_to_bits(page, &offset, eph->svhlth >> 5, 1);
        encode_int_to_bits(page, &offset, eph->svhlth, 1);
        encode_int_to_bits(page, &offset, g.week, 12);
        encode_int_to_bits(page, &offset, TOW, 20);
        encode_int_to_bits(page, &offset, 0, 23);
        break;
    ...
    case 6:
        encode_int_to_bits(page, &offset, 6, 8);
        IntValue = UnscaleInt(ionoutc->A0, -30);
        encode_double_to_bits(page, &offset, IntValue, 32);
        IntValue = UnscaleInt(ionoutc->A1, -50);
        encode_double_to_bits(page, &offset, IntValue, 24);
    ...
    encode_int_to_bits(page, &offset, 0, 40);
    encode_int_to_bits(page, &offset, 0, 22);
    encode_int_to_bits(page, &offset, 0, 2);
